...
if ($select_baocao) {
    while ($row_baocao = mysqli_fetch_assoc($select_baocao)) {
        ...
        $select_kehoachsx = mysqli_query($con, "SELECT * FROM `kehoachsx` 
                                                 WHERE `Id` IN (SELECT `IdKHSX` FROM `baocaosx` WHERE `Id` = '$id_hdnvl' )");
        if ($select_kehoachsx) {
                    ...
                    if (mysqli_num_rows($select_lohangsx) > 0) {
                        ...
                        if ($select_sp) {
                            if (mysqli_num_rows($select_sp) > 0) {
                                ...
                                $row_sp['ds_nvl'] = [];
                                $select_nvl = mysqli_query($con, "SELECT `ct_sp`.`SoLuong`, `nvl`.`TenNVL`, `nvl`.`GiaMua` FROM `ct_sp` 
