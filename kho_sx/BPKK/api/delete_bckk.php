...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if ($Id != '') {
        // Câu lệnh SQL để xóa Báo cáo từ cơ sở dữ liệu
        $sql = "DELETE FROM `bckk` WHERE `Id` = '$Id'"; // Thay 'SanPham' bằng tên bảng chứa dữ liệu Báo cáo của bạn
        ...
        if ($con) {
            if ($con->query($sql) === TRUE) {
                ...
                echo json_encode(array("message" => "Báo cáo đã được xóa thành công."));
            } else {
                // Trả về mã 500 (Internal Server Error) nếu có lỗi xảy ra khi thực thi câu lệnh SQL
                http_response_code(500);
                echo json_encode(array("message" => "Lỗi khi xóa Báo cáo: " . $con->error));
            }
        } else {
