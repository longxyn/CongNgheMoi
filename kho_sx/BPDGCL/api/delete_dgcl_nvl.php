...
if ($_SERVER["REQUEST_METHOD"] == "DELETE") { // Sửa từ GET thành DELETE
    // Nhận dữ liệu GET từ yêu cầu
    $Id = isset($_GET['id']) ? $_GET['id'] : '';

    // Kiểm tra xem Id có tồn tại không
    if ($Id != '') {
        // Câu lệnh SQL để xóa đánh giá từ cơ sở dữ liệu
        $sql = "DELETE FROM `dgcl_nvl` WHERE `Id` = '$Id'"; // Thay 'dgcl' bằng tên bảng chứa dữ liệu đánh giá của bạn
        ...
        if ($con) {
            if ($con->query($sql) === TRUE) {
                ...
                echo json_encode(array("message" => "đánh giá đã được xóa thành công."));
            } else {
                // Trả về mã 500 (Internal Server Error) nếu có lỗi xảy ra khi thực thi câu lệnh SQL
                http_response_code(500);
                echo json_encode(array("message" => "Lỗi khi xóa đánh giá: " . $con->error));
            }
        } else {
