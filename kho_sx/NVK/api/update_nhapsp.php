...
if (!empty($data)) {
    foreach ($data as $item) {
        ...
        $update_kehoachsx = mysqli_query($con, "UPDATE `dgcl` 
        JOIN `lohangsx` ON `lohangsx`.Id = `dgcl`.IdLoSX
        JOIN `sanpham` ON `lohangsx`.IdSP = `sanpham`.Id
        JOIN `kehoachsx` ON `kehoachsx`.IdLoSX = `lohangsx`.Id 
        SET `sanpham`.`ChatLuong` = 1,
            `dgcl`.`ChatLuong` = 2,
            `sanpham`.SoLuong = `dgcl`.SoLuongDat + `sanpham`.SoLuong,
            `sanpham`.NgaySX = `kehoachsx`.NgayHT,
            `sanpham`.TrangThai = 0 
        WHERE `dgcl`.Id = '$id';
        ");
